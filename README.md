...
Luego empecé a trabajar el aspecto de la sangre. Añadí un nodo Simple Noise para romper la uniformidad del color y lo combiné con el color mediante un Multiply. Al principio no veía ningún efecto, así que tuve que recurrir a Claude como asistente para auxiliarme con el problema, que resultó ser que si no están conectadas a una entrada final, las cajitas del Shader Graph no tienen efecto. Por suerte arreglar esto fue sencillo, ya que tan solo tuve que conectar el Output del Multiply con el base color del Fragment.
